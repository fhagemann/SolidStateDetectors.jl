...
function drift_charges( sim::Simulation{T}, starting_positions::Vector{CartesianPoint{T}}, energies::Vector{T};
                        #diffusion::Bool = false, self_repulsion::Bool = false, 
    ...
    return _drift_charges(   sim.detector, sim.point_types.grid, sim.point_types, starting_positions, energies,
                             get_interpolated_drift_field(sim.electric_field), sim.charge_drift_model, Δt, 
                             #diffusion, self_repulsion, 
                             max_nsteps = max_nsteps, verbose = verbose)::Vector{EHDriftPath{T}}
end
