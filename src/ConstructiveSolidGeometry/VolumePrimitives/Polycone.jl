...
struct Polycone{T,CO,N,TP<:Nothing} <: AbstractVolumePrimitive{T,CO}
    ...
    function Polycone{T,CO}(r::Union{<:AbstractArray, <:Tuple}, z::Union{<:AbstractArray, <:Tuple}, φ, origin, rotation) where {T, CO}
        ...
        _r, _z = if r[begin] != r[end] || z[begin] != z[end]
            nr += 1
            nz += 1
            (r..., r[begin]), (z..., r[end])
        else
...
function surfaces(p::Polycone{T,<:Any,N,Nothing}) where {T,N}
    s = []
    @inbounds for i in Base.OneTo(N-1)
        ...
        z1::T = p.z[i]
        z2::T = p.z[i+1]
        origin = _transform_into_global_coordinate_system(CartesianPoint{T}(zero(T), zero(T), (z1+z2)/2), p) 
        vol = if z1 == z2
            EllipticalSurface(r = (r1, r2), origin = origin, rotation = p.rotation)
        else
            ConeMantle(r = (r1,r2), hZ = abs(z1-z2)/2, origin = origin, rotation = p.rotation)
        end
        push!(s, vol)
    end
    (s...,)
...
function Dictionary(c::Polycone{T})::OrderedDict{String, Any} where {T, TR}
