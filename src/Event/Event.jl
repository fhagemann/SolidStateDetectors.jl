...
function get_signal!(evt::Event{T}, sim::Simulation{T}, contact_id::Int; Δt::RealQuantity = 5u"ns", waveform_unit::Unitful.Units = u"e_au")::Nothing where {T <: SSDFloat}
    ...
    evt.waveforms[contact_id] = get_signal(sim, evt.drift_paths, flatview(evt.energies), contact_id; Δt, waveform_unit)
    nothing
end
...
* `waveform_unit::Unitful.Units = u"e_au"`: Unit of the returned waveform (charge or energy).
...
"""
function get_signals!(evt::Event{T}, sim::Simulation{T}; Δt::RealQuantity = 5u"ns", waveform_unit::Unitful.Units = u"e_au")::Nothing where {T <: SSDFloat}
    @assert !ismissing(evt.drift_paths) "No drift path for this event. Use `drift_charges!(evt::Event, sim::Simulation)` first."
    if ismissing(evt.waveforms)
        evt.waveforms = Union{Missing, RadiationDetectorSignals.RDWaveform}[missing for i in eachindex(sim.detector.contacts)]
    end
    for contact in sim.detector.contacts
        ...
        if !ismissing(sim.weighting_potentials[contact.id])
            evt.waveforms[contact.id] = get_signal(sim, evt.drift_paths, flatview(evt.energies), contact.id; Δt, waveform_unit)
        end
    end
    nothing
end
...
"""
function simulate!(evt::Event{T}, sim::Simulation{T}; max_nsteps::Int = 1000, Δt::RealQuantity = 5u"ns", diffusion::Bool = false, self_repulsion::Bool = false, waveform_unit::Unitful.Units = u"e_au", end_drift_when_no_field::Bool = true, geometry_check::Bool = false, verbose::Bool = true)::Nothing where {T <: SSDFloat}
    drift_charges!(evt, sim; max_nsteps, Δt, diffusion, self_repulsion, end_drift_when_no_field, geometry_check, verbose)
    get_signals!(evt, sim; Δt, waveform_unit)
    nothing
end
...
@recipe function f(gdd::Plot_electron_and_hole_contribution; linewidth = 2, n_samples = missing)
    ...
    waveform_unit::Unitful.Units = unit(evt.waveforms[contact_id])
