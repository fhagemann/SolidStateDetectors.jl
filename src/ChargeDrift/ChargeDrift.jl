...
function _drift_charges(detector::SolidStateDetector{T}, grid::Grid{T, 3}, point_types::PointTypes{T, 3},
    ...
    n_e::Int = _drift_charge!(drift_path_e, timestamps_e, detector, point_types, grid, starting_points, energies./T(ustrip(detector.semiconductors[1].material.E_ionisation)), dt, electric_field, cdm, Electron, verbose = verbose) # diffusion, self_repulsion, verbose = verbose)
    n_h::Int = _drift_charge!(drift_path_h, timestamps_h, detector, point_types, grid, starting_points, energies./T(ustrip(detector.semiconductors[1].material.E_ionisation)), dt, electric_field, cdm, Hole, verbose = verbose) # diffusion, self_repulsion, verbose = verbose)
...
function _add_fieldvector_drift!(step_vectors::Vector{CartesianVector{T}}, current_pos::Vector{CartesianPoint{T}}, 
    ...
    for n in eachindex(step_vectors)
        if !done[n]
            step_vectors[n] += get_velocity_vector(velocity_field, _convert_point(current_pos[n], S))
