...
@testset "Test primitive parsing" begin
    ...
    @testset "Polycone" begin
        polycone1 = @inferred CSG.Polycone(CSG.ClosedPrimitive,r=Float32[0,2,4,0,0],z=Float32[0,1,2,3,0],origin=zero(CartesianPoint{Float16}),rotation=one(SMatrix{3, 3, Float16, 9}))
        polycone2 = @inferred CSG.Polycone{Float32}(r = [0,2,4,0], z = [0,1,2,3]) # omit last entry
        ...
        polycone3 = Geometry(Float32,dict,default_units,no_translations)
        @test polycone1 == polycone3
        @test dict == Dictionary(polycone3)

        polycone_open = @inferred CSG.Polycone(CSG.OpenPrimitive,r=Float32[0,2,4,0,0],z=Float32[0,1,2,3,0],origin=zero(CartesianPoint{Float16}),rotation=one(SMatrix{3, 3, Float16, 9}))
